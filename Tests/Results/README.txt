...
- Two-Sided: the filtering (using myfiltfilt) is applied to the signal
  zero-padded at both sides. This is the method used in the ANSI Toolbox.
  ...
  resulting in energy leakage over time towards the two ends of the signal.
- One-Sided: the filtering (using myfiltfilt) is applied to the signal
  zero-padded to the right-side only. This example is only for testing. 
  The comparison between the Two-Sided and One-Sided figures shows that 
